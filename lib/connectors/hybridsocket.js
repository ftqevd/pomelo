...
var Socket = function(id, socket) {
  ...
  if(!socket._socket) {
    ...
    var ipAddr = socket._socket.remoteAddress;
    
    if(socket.upgradeReq && socket.upgradeReq.headers['x-forwarded-for']) {
      ipAddr = socket.upgradeReq.headers['x-forwarded-for'].split(',')[0].trim();
    }
